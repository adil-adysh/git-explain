...
pub fn render(items: &[ExplainedFunction], context: &AnalysisContext) -> String {
    ...
    for (i, x) in items.iter().enumerate() {
        h.push_str(&format!(r#"<section><h2>{}</h2><p>Changed unit: <strong>{:?}</strong> <code>{}</code>, lines {}-{}.</p><article><h3>{}</h3><p><strong>{:?}</strong></p><h4>Overview</h4><p class="explanation">{}</p><h4>Code and explanation</h4>"#,escape(&x.file),x.unit.kind,escape(&x.unit.name),x.unit.start_line,x.unit.end_line,escape(&x.unit.name),x.unit.kind,escape(&x.explanation.overview)));
        ...
        h.push_str(&format!(r#"<button type="button" data-index="{}" class="deep">Explain this code in depth</button><section id="deep-{}" hidden><h4>Detailed explanation</h4><p></p></section></article></section>"#,i,i));
    }
